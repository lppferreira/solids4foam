...
) const
{
    ...
    const tensorField& FinvBf = Finvf_.boundaryField()[patchID];

    // Patch total Jacobian
    const scalarField& JBf = Jf_.boundaryField()[patchID];
...
tmp<scalarField> unsThermalNonLinGeomTotalLagSolid::patchKappaDelta
(
    const label patchID
) const
{
