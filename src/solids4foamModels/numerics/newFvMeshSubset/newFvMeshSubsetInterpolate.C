...
) const
{
    ...
    forAll (patchFields, patchI)
    {
        ...
        if (pm[patchI] == -1)
        {
            ...
            (
                patchI,
                calculatedFvsPatchField<Type> // JN: Compiler likes this // PC: bugfix
...
) const
{
	...
	// JN: Leave this out until we find the problem here
/*        if (pm[patchI] == -1)
        {
            patchFields.set
            (
...
////        {
	...
	//JN: Compiler cries so leave it out for now
/*            patchFields.set
            (
                patchI,
 ...
 ////   }
	...
	// JN: Not available in OF 3.0, we'll have to find another solution!
