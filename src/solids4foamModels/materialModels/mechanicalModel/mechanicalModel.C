...
//            ZoneID<cellZone>(cellZoneNames_[lawI], mesh().cellZones());
	const cellZoneID cellZone
        (
            cellZoneNames_[lawI],
            mesh().cellZones()
        );

/*        const zone<cellZone> cellZoneID =
            zone<cellZone>(cellZoneNames_[lawI], mesh().cellZones());*/
        ...
        const labelList& curCellZone = mesh().cellZones()[cellZone.index()];
...
//Foam::PtrList<Foam::fvMeshSubset>& Foam::mechanicalModel::subMeshes()
{
    if (subMeshes_.empty())
    {
        makeSubMeshes();
...
//const Foam::PtrList<Foam::newLeastSquaresVolPointInterpolation>&//volPointInterpolation
...
//    const PtrList<fvMeshSubset>& subMeshes = this->subMeshes();
    ...
    if (interfaceBaseFaces().size() == 0)
    {
        ...
        forAll(subMeshes, lawI)
        {
            if (D.name() == "D")
            {
////                subMeshD()[lawI] = subMeshes[lawI].interpolate(D);// JN: Compiler cries!
            }
            else
            {
////                subMeshDD()[lawI] = subMeshes[lawI].interpolate(D);// JN: Compiler cries!
            }
        }
...
////        subMeshD = subMeshes[lawI].interpolate(D);// JN: Compiler cries!
