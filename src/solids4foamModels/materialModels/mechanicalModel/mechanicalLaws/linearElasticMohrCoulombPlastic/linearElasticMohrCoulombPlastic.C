    ...
    else
    {
        ...
        else
        {
            ...
            if (R2 < Q3)
            {
                ...
                ii =
                    m2SqrtQ*Foam::cos((theta + Foam::constant::mathematical::twoPi)/3.0)
                  - aBy3;
                iii =
                    m2SqrtQ*Foam::cos((theta - Foam::constant::mathematical::twoPi)/3.0)
                  - aBy3;
...
:
    ...
    (
        (
            (1 + sin(varPhi_/180.0*Foam::constant::mathematical::pi))
           /(1 - sin(varPhi_/180.0*Foam::constant::mathematical::pi))
        ).value()
    ),
    m_
    (
        (
            (1 + sin(varPsi_/180.0*Foam::constant::mathematical::pi))
           /(1 - sin(varPsi_/180.0*Foam::constant::mathematical::pi))
        ).value()
    ),
    a_(k_, 0, -1),
...
)
{
    ...
    // Correct the coupled boundary conditions
	//JN: Doesn't seem to exist for surfaceSymmTensorFields
//    sigma.correctBoundaryConditions();
    activeYield_.correctBoundaryConditions();
