...
// MUMPS files
#include "/usr/local/include/dmumps_c.h"
#include <vector>
...
{
    Info<< this->typeName << ": direct solver" << endl;
    ...
    // id.sym = 1; // for symmetric positive definite (faster)
    // id.sym = 2; // for general symmetric
    ...
    Info<< this->typeName << ": converting lduMatrix to MUMPS format" << endl;
    ...
    // Insert diagonal
    // Note: MUMPS indexing starts from 1 not from 0
    int ID = -1;
    int k = 0;
    forAll(d, dI)
    {
    ...
    if (id.info[0] > 0)
    {
        ...
        (
            ...
            "    const Field<Foam::vector>& blockB\n"
            ")"
        )   << "The MUMPS solver returned with a warning:" << nl
            << "id.info[0]: " << id.info[0] << nl
            << "id.info[1]: " << id.info[1] << nl
            << "Please see the MUMPS user guide for interpretation" << endl;
    }
    else if (id.info[0] < 0)
    {
        FatalErrorIn
        (
            ...
            "    const Field<Foam::vector>& blockB\n"
            ")"
        )   << "The MUMPS solver returned with an error:" << nl
            << "id.info[0]: " << id.info[0] << nl
            << "id.info[1]: " << id.info[1] << nl
            << "Please see the MUMPS user guide for interpretation" << endl;
    }
    ...
    Info<< this->typeName << ": copying results into volField format" << nl
        << endl;
